...
def is_test_time(grade: str) -> bool:
    schedule = TEST_SCHEDULE.get(grade)
    if not schedule:
        return True
    import datetime
    now_utc = datetime.datetime.utcnow()
    now_uz  = now_utc + datetime.timedelta(hours=5)
    start_h, start_m, end_h, end_m = schedule
    start = now_uz.replace(hour=start_h, minute=start_m, second=0, microsecond=0)
    end   = now_uz.replace(hour=end_h,   minute=end_m,   second=0, microsecond=0)
    return start <= now_uz < end

def get_test_time_str(grade: str) -> str:
    schedule = TEST_SCHEDULE.get(grade)
    if not schedule:
        return "istalgan vaqt"
    return f"{schedule[0]:02d}:{schedule[1]:02d} - {schedule[2]:02d}:{schedule[3]:02d}"
...
async def finish_test(chat_id, state):
    ...
    # VAB qo'shish: har to'g'ri javob uchun 2 VAB
    ...
    tag = "💎 Pullik test" if test_type == "paid" else "🆓 Bepul test"
...
async def free_tests_menu(message: types.Message):
    ...
    if user["test_started"]:
        await message.answer(
            f"⚠️ Siz {grade} bepul testini allaqachon ishlagansiz!\n\n"
            f"💎 Pullik testlarga kirish uchun — 'Pullik testlar' bo'limiga o'ting.",
            reply_markup=main_menu_keyboard()
        )
        return

    schedule_info = get_test_time_str(grade)
    if not is_test_time(grade):
        await message.answer(
            f"⏰ {grade} testi vaqti: {schedule_info}\n"
            f"Hozir test vaqti emas. Keyinroq keling!",
            reply_markup=main_menu_keyboard()
        )
        return
    ...
    await message.answer(
        f"🆓 Bepul test: {grade}\n"
        f"📝 Savollar: {len(TESTS.get(grade, []))} ta\n"
        f"⏱ Vaqt: 45 daqiqa\n"
...
async def paid_tests_menu(message: types.Message):
    ...
    grade   = user["grade"]
    vab     = user["vab"]
    has_it  = has_paid_test(user["id"], grade)

    if has_it:
        # Pullik test allaqachon sotib olingan, boshlash
        await message.answer(
            f"💎 Pullik test: {grade}\n"
            f"📝 Savollar: {len(PAID_TESTS.get(grade, []))} ta qiyin savol\n"
            f"⏱ Vaqt: 45 daqiqa\n\n"
            f"Boshlashga tayyormisiz?",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💎 Pullik testni boshlash",
                                      callback_data=f"start_paid_{user['id']}")]
            ])
        )
    else:
        # Sotib olish
        await message.answer(
            f"💎 Pullik testlar haqida:\n\n"
            f"✅ Qiyinroq va ko'proq savollar\n"
            f"✅ Har to'g'ri javob: +{VAB_PER_CORRECT_ANSWER} VAB\n"
            f"✅ Cheksiz marta ishlash imkoni\n\n"
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"💳 Pul to'lash ({PAYMENT_AMOUNT:,} so'm)",
                                      callback_data=f"pay_money_{user['id']}")],
                [InlineKeyboardButton(text=f"💰 VAB sarflash ({VAB_FOR_TEST_PURCHASE} VAB)",
                                      callback_data=f"pay_vab_{user['id']}")],
            ])
        )
...
async def pay_with_money(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"⚠️ To'lov izohiga: {user['full_name']} — {user['grade']}\n\n"
        f"To'lov chekini (screenshot) shu yerga yuboring 👇\n"
        f"Bekor qilish: /cancel"
    )
    await state.update_data(paying_user_id=p_id, paying_grade=user["grade"])
    ...
    try:
        await bot.send_photo(
            ...
            caption=(
                ...
                f"📞 Tel: {user['phone']}\n"
                f"💰 Summa: {PAYMENT_AMOUNT:,} so'm\n\n"
                f"Tasdiqlash: /confirm {order_id}\n"
                f"Rad etish: /reject {order_id}"
            )
        )
    except Exception as e:
...
async def pay_with_vab(callback: types.CallbackQuery):
    ...
    if success:
        ...
        await callback.message.answer(
            f"✅ Pullik test aktivlashtirildi!\n"
            f"💰 -{VAB_FOR_TEST_PURCHASE} VAB sarflandi\n"
            f"💼 Qolgan VAB: {user['vab']} VAB\n\n"
            f"💎 {grade} pullik testini hozir ishlashingiz mumkin!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💎 Boshlash",
                                      callback_data=f"start_paid_{p_id}")]
            ])
        )
    else:
...
async def start_free_test(callback: types.CallbackQuery, state: FSMContext):
    ...
    if user["test_started"]:
        await callback.answer("Siz bepul testni allaqachon ishlagansiz!", show_alert=True)
        return
    ...
    update_user_field(user["id"], test_started=1)
...
async def start_paid_test(callback: types.CallbackQuery, state: FSMContext):
    ...
    q_list = PAID_TESTS.get(grade, [])
    if not q_list:
        await callback.message.answer(f"{grade} pullik savollar yo'q.")
        await callback.answer()
        return

    await _start_test_session(callback, state, user, q_list, "paid")

async def _start_test_session(callback, state, user, q_list, test_type):
    grade = user["grade"]
    tag   = "💎 Pullik" if test_type == "paid" else "🆓 Bepul"
