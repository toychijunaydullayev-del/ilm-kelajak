...
# ═══════════════════════════════════════════════
# SINF BO'YICHA TEST VAQTLARI (UTC+5)
# ═══════════════════════════════════════════════
TEST_SCHEDULE = {
    "1-sinf": (17, 0, 18, 0),
    "2-sinf": (17, 0, 18, 0),
...
def is_test_time(grade: str) -> bool:
    """Hozir shu sinf uchun test vaqti ekanligini tekshiradi (UTC+5)"""
    schedule = TEST_SCHEDULE.get(grade)
    if not schedule:
        return True  # Jadvalda yo'q sinf uchun har doim ochiq
    import datetime
...
def get_test_time_str(grade: str) -> str:
    """Sinf uchun test vaqtini matn ko'rinishida qaytaradi"""
    schedule = TEST_SCHEDULE.get(grade)
    if not schedule:
...
async def start_test(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not is_test_time(grade):
        test_time = get_test_time_str(grade)
        await callback.answer(
            f"⏰ Sizning test vaqtingiz: {test_time}\n"
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if existing:
        await message.answer(
            f"Siz allaqachon ro'yxatdan o'tgansiz, Tes faqat bir marta ishlanadi hurmatli {existing['full_name']}!")
        return
...
# ═══════════════════════════════════════════════
# ADMIN: KESHNI YANGILASH
# ═══════════════════════════════════════════════
@dp.message(Command("refresh"), F.from_user.id == ADMIN_ID)
async def refresh_cache_cmd(message: types.Message):
    old_count = get_users_count()
    load_users_to_cache()
    new_count = get_users_count()
    await message.answer(
        f"✅ Baza yangilandi!\n"
        f"👤 Oldin: {old_count} ta\n"
        f"👤 Hozir: {new_count} ta ishtirokchi"
    )
