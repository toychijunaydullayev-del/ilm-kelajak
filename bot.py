...
TESTS = {
    ...
    },
    {
      ...
      "options": {
        "A": "Faqat changchili",
        "B": "Ham changchi, ham urug'chisi bor",
        "C": "Gulkosabargsiz",
        "D": "Faqat urug'chili"
      },
      "answer": "B",
    ...
    },
    {
      ...
      "options": {
        "A": "Qora teri quyosh issiqligini yaxshi o'tkazadi",
    ...
    },
    {
      ...
      "options": {
        "A": "Spirt og'irroq",
        "B": "Spirt zarrachalari tortishish kuchsizroq",
        "C": "Spirt yengil",
        "D": "Spirt issiqroq"
      },
      "answer": "B",
    ...
    {
      "question": " Ko'priklarda nima uchun bo'shliq qoldiriladi?",
      "options": {
        "A": "Suv oqishi uchun",
        "B": "Bezak uchun",
        "C": "Sovuqda metall qisqaradi",
        "D": "Konstruksiyani yorib yubormasligi uchun"
      },
      "answer": "D",
      "chapter": "3-bob",
      "difficulty": "medium"
    ...
    {
      "question": "Yo'lga qish faslida tuz sepilishi sababi?",
      "options": {
        "A": "Bezak uchun",
    ...
    {
      "question": "Parashyutchi doimiy tezlikda tushayotganda qanday kuchlar ta'sir etadi ?",
      "options": {
    ...
    {
      "question": "Samolyot qanotining yuqori qismi qavariq bo'lish nima uchun kerak?",
      "options": {
        "A": "Ishqalanish uchun",
        "B": "Ko'rinish uchun",
        "C": "Oson ko'tarilish uchun",
        "D": "Og'irlik uchun"
      },
      "answer": "C",
      "chapter": "5-bob",
    ...
    },
    {
      ...
      "options": {
        ...
        "C": "Bir xil",
        "D": "Magnit magnetik materialni tortadi"
      },
      "answer": "D",
      "chapter": "7-bob",
      "difficulty": "medium"
    ...
    },
    {
      ...
      "options": {
        "A": "Magnetlanib qoladi",
        "B": "Hech narsa bo'lmaydi",
        "C": "Magnitli chiziq ma'lumotlari o'chib ketishi mumkin",
    ...
    },
    {
      ...
      "options": {
        "A": "Transpiratsiya",
        "B": "Manzara",
        "C": "Yog'ochlik olinadi",
        "D": "Chorvachilikda ishlatiladi"
      },
      "answer": "A",
      "chapter": "8-bob",
      "difficulty": "medium"
    ...
    },
    {
      ...
      "options": {
        "A": "Suvning muzlashi",
        "B": "Suvning qaynashi",
        "C": "Suvga ortiqcha ozuqa tushishi",
    ...
    },
    {
      ...
      "options": {
        "A": "Inson oyoq izi",
        "B": "Ko'mir rangi",
        "C": "Inson yoki tashkilot chiqariladigan CO2 umumiy miqdori",
        "D": "Tuproqdagi karbon"
      },
      "answer": "C",
      "chapter": "9-bob",
    ...
    },
    {
      ...
      "options": {
        "A": "Ehtiyojlarini muvozanatli qondirish",
        "B": "Faqat iqtisodiy",
        "C": "Faqat texnologik",
        "D": "Faqat sanoat"
      },
...
def init_excel():
    if not os.path.exists(EXCEL_FILE):
        ...
        ws.append(["ID", "Telegram ID", "Ism Familiya",
                   "Maktab", "Sinf", "Telefon", "Ball", "Test boshlandi"])
...
def add_user(telegram_id, full_name, school, grade, phone):
    ...
    ws = wb.active
    # Oldin ro'yxatdan o'tganmi yoki yo'q tekshiramiz
    ...
    new_id = ws.max_row
    # Excelga yozamiz
    ws.append([new_id, telegram_id, full_name, school, grade, phone, 0, 0])
    wb.save(EXCEL_FILE)

    # MUHIM: Yangi o'quvchini srazi tezkor xotiraga ham qo'shib qo'yamiz
    USERS_CACHE[new_id] = {
...
# 1. Excelni faqat 1 marta o'qiydigan funksiya (Bot yonayotganda ishlaydi)
def load_users_to_cache():
    ...
    try:
        ...
        ws = wb.active
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row[0]: # ID bo'sh bo'lmasa
                USERS_CACHE[row[0]] = {
                    "id": row[0], "telegram_id": row[1],
                    "full_name": row[2], "school": row[3],
                    "grade": row[4], "phone": row[5],
                    "score": row[6] or 0,
                    "test_started": row[7] or 0,
                }
        print(f"✅ Baza yuklandi: {len(USERS_CACHE)} ta o'quvchi tayyor.")
    except Exception as e:
        print(f"❌ Excelni o'qishda xato: {e}")

# 2. Xotiradan tezkor qidirish (Excel ID bo'yicha)
def get_user_by_id(user_id):
    return USERS_CACHE.get(user_id)

# 2b. Telegram ID bo'yicha qidirish
...
# 3. Foydalanuvchini yangilash (Xotirani srazi yangilab, Excelga fonga yozadi)
def update_user(user_id, score=None, test_started=None):
    # Oldin tezkor xotirani yangilaymiz (bot qotmasligi uchun)
    ...
    # Keyin xotirjam Excelga yozib qo'yamiz
    try:
        wb = openpyxl.load_workbook(EXCEL_FILE)
        ws = wb.active
        for row in ws.iter_rows(min_row=2):
            if row[0].value == user_id:
                if score is not None:
                    row[6].value = score
                if test_started is not None:
                    row[7].value = test_started
                break
        wb.save(EXCEL_FILE)
    except Exception as e:
        print(f"Yangilashda xato: {e}")

# 4. Barcha Telegram ID larni Exceldan emas, xotiradan sanash
def get_all_telegram_ids():
    return [user["telegram_id"] for user in USERS_CACHE.values() if user["telegram_id"]]

# 5. O'quvchilar sonini xotiradan aniqlash
def get_users_count():
...
async def finish_test(chat_id: int, state: FSMContext):
    ...
    result_text = (
        ...
        f"Yakuniy natijalarni bilish uchun rasmiy kanalimizga qo'shiling\n biz bilan bog'lanish @IlmNuri_admi 👇"
    )
